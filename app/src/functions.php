...
function rmbt_renoteck_scripts() {

	wp_enqueue_style( 'swiper-bundle', get_template_directory_uri() . '/assets/styles/libs/swiper-bundle.min.css', array(), '1.0', 'all' );
	wp_enqueue_style( 'rmbt_renoteck-main', get_template_directory_uri() . '/assets/styles/main-style.min.css', array(), '1.0', 'all' );

	wp_enqueue_script( 'swiper-bundle', get_template_directory_uri() . '/assets/js/libs/swiper-bundle.min.js', array(), '', true );
	wp_enqueue_script( 'rmbt_renoteck-app', get_template_directory_uri() . '/assets/js/app.main.min.js', array( 'jquery' ), '1.0', true );
...
add_action( 'wp_enqueue_scripts', 'rmbt_renoteck_scripts', 20 );
...
function rmbt_site_setup() {
	...
	load_theme_textdomain( 'rmbt_renoteck', get_template_directory() . '/languages' );
	add_theme_support( 'automatic-feed-links' );
}
add_action( 'after_setup_theme', 'rmbt_site_setup' );

function simple_rmbt_renoteck_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'simple_rmbt_renoteck_content_width', 640 );
}
add_action( 'after_setup_theme', 'simple_rmbt_renoteck_content_width', 0 );
...
function rmbt_widgets_init() {
	register_sidebar(
		array(
			'name' => esc_html__( 'Sidebar For Blog page', 'rmbt_renoteck' ),
			'id' => 'rmbt_blog_sidebar',
			'description' => esc_html__( 'Add widgets here', 'rmbt_renoteck' ),
	...
	register_sidebar(
		array(
			'name' => esc_html__( 'Sidebar For Shop page', 'rmbt_renoteck' ),
...
function rmbt_change_menus_items( $args, $item ) {
	...
	if ( $args->theme_location === 'food_menu' ) {
		if ( class_exists( 'ReduxFramework' ) && in_array( 'menu-item-type-post_type_archive', $item->classes ) ) {
			$args->before = '<img src="' . $rmbt_renoteck_options['restaurant_menu-section_icon_first_item_menu']['url'] . '" alt="">';
		} else {
			if ( class_exists( 'ACF' ) ) {
